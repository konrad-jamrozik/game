...
{
    public readonly List<GameEvent> GameEventsUntilStartGameState;
    public readonly GameState StartGameState;
    
    public readonly List<GameEvent> GameEventsUntilGameState;
    public readonly GameState GameState;

    public GameSessionTurn2(
        List<GameEvent>? gameEventsUntilStartGameState = null,
        GameState? startGameState = null,
        List<GameEvent>? gameEventsUntilGameState = null,
        GameState? gameState = null)
    {
        GameEventsUntilStartGameState = gameEventsUntilStartGameState ?? new List<GameEvent>();
        StartGameState = startGameState ?? GameState.NewInitialGameState();
        GameEventsUntilGameState = gameEventsUntilGameState ?? new List<GameEvent>();
    ...
    {
        //Contract.AssertImplication(GameEventsUntilStartGameState.Any(), GameEventsUntilGameState.First() is AdvanceTimePlayerAction);
    }
